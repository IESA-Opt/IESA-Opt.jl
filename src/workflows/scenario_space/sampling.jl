...
function _sample_lhs(spec::CampaignSpec, params::Vector{String}, bounds::Vector{Tuple{Float64,Float64}})
    ...
    for j in 1:k
        ...
        perm = randperm(rng, n)
        for i in 1:n
            stratum = perm[i] - 1                 # 0..n-1
            t = (stratum + u[i]) / n              # in (0, 1)
...
"""
    _sample_sobol(spec, params, bounds) -> SampleMatrix

Generate `n_variants` Sobol points in `[0,1]^k`, skipping the all-zeros origin
(standard convention), then map each column linearly into its parameter
bounds. Sobol.jl is deterministic given the same dimension and call sequence,
so the seed only matters when we later randomise scrambling (not used here —
...
function _sample_sobol(spec::CampaignSpec, params::Vector{String}, bounds::Vector{Tuple{Float64,Float64}})
    ...
    k = length(params)
    seq = SobolSeq(k)
    # Conventionally skip the (0,0,...,0) starting point.
    Sobol.skip(seq, 1)
    values = Matrix{Float64}(undef, n, k)
    for i in 1:n
        u = next!(seq)
...
function _sample_morris(spec::CampaignSpec, params::Vector{String}, bounds::Vector{Tuple{Float64,Float64}}; p::Int = 4)
    ...
    r = spec.n_variants
    k = length(params)
    Δ = p / (2 * (p - 1))               # canonical Morris step
    grid_levels = 0:(1/(p-1)):1          # p evenly spaced levels in [0,1]
    n_rows_per_traj = k + 1
    total_rows = r * n_rows_per_traj
    values = Matrix{Float64}(undef, total_rows, k)
    row = 0
    for _ in 1:r
        # Random starting point on the grid (chosen so each coordinate can move ±Δ within [0,1])
        x0 = Vector{Float64}(undef, k)
        for j in 1:k
            # Allow x0 in {0, 1/(p-1), ..., 1-Δ} so that x0 + Δ stays ≤ 1, with random sign.
            n_levels_safe = max(1, length(grid_levels) - Int(round(Δ * (p - 1))))
            x0[j] = grid_levels[rand(rng, 1:n_levels_safe)]
        end
        # Random parameter ordering for this trajectory
        order = randperm(rng, k)
        x = copy(x0)
        # First row of the trajectory = base point
        row += 1
        for j in 1:k
            lo, hi = bounds[j]
            values[row, j] = lo + x[j] * (hi - lo)
        end
        # k subsequent rows, each perturbing one parameter by ±Δ (sign chosen to stay in [0,1])
        for step in 1:k
            j_perturb = order[step]
            sign = (x[j_perturb] + Δ <= 1.0) ? 1.0 : -1.0
            x[j_perturb] += sign * Δ
            # Clamp defensively (numerical noise only)
            x[j_perturb] = clamp(x[j_perturb], 0.0, 1.0)
            row += 1
            for j in 1:k
                lo, hi = bounds[j]
                values[row, j] = lo + x[j] * (hi - lo)
